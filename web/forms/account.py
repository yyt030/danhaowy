...
class SigninForm(Form):
    """登陆表单"""
    username = TextField('登录名:', validators=[DataRequired(u'登录名不能为空')], default='', description=u'用户名、邮件地址或手机号')
    ...
    def validate_password(self, field):
        ...
        print "admin", name
        user = User.query.filter(or_(User.name == name, User.email == name, User.mobile)).first()
        if not user:
...
class AdminloginForm(Form):
    """登陆表单"""
    admin_name = TextField('登录名:', validators=[DataRequired(u'登录名不能为空')], default='', description=u'用户名、QQ、邮件地址或手机号')
    password = PasswordField('密码:', validators=[DataRequired(u'密码不能为空')], description=u'密码')
...
class RegisterForm(Form):
    ...
    code = TextField('验证码:', validators=[DataRequired('验证码不能为空')])
